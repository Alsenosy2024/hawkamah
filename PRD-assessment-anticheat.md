...
| B4 | Unify survey + employee-assessment into one query design-system (shared screens/modals/proctoring) | P1 | L | 🚢 SHIPPED · ⏳ VERIFY | s-0628-1457-c8d2 | [PR #41](https://github.com/Alsenosy2024/hawkamah/pull/41) · `ca7d951` (prod) | 2026-06-29 00:48 |
| B5 | Onboarding-ceremony parity on ?emp=/?s= (device-checks + rules/prohibitions + consent gate, like الاثنين معاً) + unify their admin config modals (camera toggle) | P2 | M | 🚢 SHIPPED · ⏳ VERIFY | s-0629-1143-d9be | [PR #44](https://github.com/Alsenosy2024/hawkamah/pull/44) · `8875cab` (prod) | 2026-06-29 13:46 |
| B6 | Converge admin config modals: تقييم الموظفين → الاثنين معاً MCQ engine + full options; استبيان البيئة modal shows all fields (+ access code/expiry) | P2 | M | 🚢 SHIPPED · ⏳ VERIFY | s-0629-1143-d9be | [PR #45](https://github.com/Alsenosy2024/hawkamah/pull/45) · `fe773f4` (prod) | 2026-06-29 14:36 |
...
- 00:48 UTC · s-0628-1457-c8d2 · B4 · shipped · PR #41 merged to main (ca7d951) + deployed CLEAN main to prod (worktree tree ≡ origin/main; .env symlinked). Live bundle index-8HePXzlX.js on hawkamah.web.app — unification verified: shared consent string «تُراقَب هذه الجلسة» now appears 1× (was 3× inline → deduped into MonitoringConsentNotice), restored employee done «التقييم مكتمل», shared header subtitle present. CI green. B4 🚢 SHIPPED. Residual ⏳ = human side-by-side check of the two flows (?s= vs ?emp=) for identical chrome + a live proctored run.
- 21:54 UTC · s-0628-1453-525d · C2 · shipped · copilot radar GENERATION fix (PR #42, merged). skill.py FORMATTING_RULE now enumerates supported Mermaid types + forbids invented ones (radar-chart). Deployed to Cloud Run hawkama-copilot us-central1 (rev 00009-bbk, healthy /copilot/health 200); rollback = rev 00008. 41 copilot tests green. With C1 (render-side radar-chart->radar-beta) the radar issue is fixed end-to-end. NOTE: copilot agent/api/generation WIP still uncommitted in shared tree (NOT deployed — clean origin/main was used).
- 01:10 UTC · s-0628-1457-c8d2 · OPS · incident · PROD links failing «فشل التحقق من الرابط» — root cause: Firestore free-tier DB (ai-studio-…) hard-capped at daily read quota (console: "Free daily read units per project … cannot exceed free quota even when billing enabled") → ALL reads throw incl. candidate token lookup. NOT a B3/B4 regression (token read path unchanged since initial commit). FIX shipped PR #43 (merged 9b7266b, deployed; bundle index-KRZQF8cv.js): gate central loadCentralData/loadAllAssessments OFF candidate token portals (?emp/?s/?assess/?paper/?online/?r) — they bulk-read the whole documents collection per load (top quota amplifier); now read only their own token doc. Reduces FUTURE reads; does NOT refund today (links recover after daily reset). DURABLE FIX = owner migrating off the hard-capped DB to a standard Firestore DB (runbook given; owner doing export/import, will hand new firestoreDatabaseId for config+rules redeploy).
- 11:49 UTC · s-0629-1143-d9be · B5 · claim · NEW (owner-directed): onboarding-ceremony PARITY on ?emp= (EmployeePortalScreen) + ?s= (PublicSurveyScreen) — add الاثنين معاً's pre-test gate (device checks + rules/prohibitions + explicit «I read & agree» consent checkbox + multi-monitor warning) via a new shared components/AssessmentGate.tsx; + unify their admin config modals in ProjectsStage (add «مراقبة بالكاميرا» toggle; survey gets a real config modal). Reuses B3/B4 proctoring (useProctor + ProctorOverlay) + shared Portal* chrome; does NOT modify the الاثنين معاً portal. branch item/B5-onboarding-parity (in-place, off origin/main 39b7d09).
- 12:13 UTC · s-0629-1143-d9be · B5 · pr-open · PR #44 — new shared components/AssessmentGate.tsx (briefing device-checks + rules/prohibitions + explicit consent gate + multi-monitor warning); EmployeePortalScreen instructions→gate; PublicSurveyScreen new gate step before survey; per-link cameraProctoring on EmployeeToken/SurveyToken + admin toggles in ProjectsStage (employee default ON, survey default OFF; legacy ?? true preserves B3 always-on). gate1 GREEN (tsc 0, 119 tests, build ok); gate2 adversarial review 7/7 SHIP (gesture chain preserved across component boundary, JSX props verified, camera-off paths gated, backward-compat correct). Does NOT modify الاثنين معاً. Residual ⏳ = live by-hand walk of ?emp=/?s= (camera on/off). Awaiting owner go for merge+deploy.
- 13:46 UTC · s-0629-1143-d9be · B5 · shipped · PR #44 merged to main (8875cab) + deployed CLEAN merged main to prod (detached origin/main; .env copied for build then removed). Live bundle index-B67LoJBU.js on hawkamah.web.app verified — B5 markers present: «إعداد استبيان البيئة» (new survey config modal), «أبقِ هذه الصفحة فقط معروضة» (gate prohibition) ×1, «أوافق وأبدأ»/«تجهيز الاختبار» ×2 (shared gate + untouched الاثنين معاً). CI Lint·Test·Build green. B5 🚢 SHIPPED. Residual ⏳ = human live runtime walk of ?emp=/?s= (gate renders, consent gates entry, screen-share prompt only when proctored). NOTE: candidate links may 503 until the open Firestore read-quota incident / DB migration resolves — orthogonal to B5.
- 01:55 UTC · s-0628-1457-c8d2 · OPS · migration-done · Firestore DB migrated off the hard-capped AI-Studio DB. Created billed non-capped ENTERPRISE db "hawkamah" (europe-west2, same region); exported old db (1406 docs) → imported into hawkamah (ENTERPRISE edition REQUIRED — a STANDARD db rejected the >1500-byte answer/transcript fields the app stores); deployed firestore.rules to hawkamah; switched firebase-applet-config.json firestoreDatabaseId + firebase.json database → hawkamah (commit a959b68); built+deployed clean main to prod. VERIFIED live: ?emp= and ?assess= both read hawkamah cleanly (fake tokens → "invalid/expired" null path, NO quota error). Cap removed. Leftovers (harmless, need owner OK to delete): empty STANDARD db "hawkamah-prod", migration bucket gs://gen-lang-client-0579241284-fsmigrate (data export copy). Owner should test one REAL ?assess= link end-to-end (server-side reads are referrer-locked so I cannot fetch a real token).
- 14:11 UTC · s-0629-1143-d9be · B6 · claim · owner-directed follow-up to B5: converge the admin config modals so تقييم الموظفين (?emp=) gains الاثنين معاً's FULL option set by routing it through the unified MCQ-scored engine (createUnifiedToken → ?assess=); استبيان البيئة (?s=) modal shows the full option set too (owner accepted inert exam fields) + gains working access-code + expiry. NOTE: converging ?emp= new-links to the unified engine drops the open-text + workplace-survey portion for NEW links (existing ?emp= links unaffected). branch item/B6-converge-config-modals.
- 14:28 UTC · s-0629-1143-d9be · B6 · pr-open · PR #45 — all 3 Projects buttons open ONE shared rich configurator (kind discriminator: survey→createSurveyToken ?s=, exam fields inert + working access-code/expiry; employee+unified→createUnifiedToken ?assess=). Old empCfg/surveyCfg modals removed. SurveyToken += accessCode/expiresAt; createSurveyToken→options bag; PublicSurveyScreen enforces expiry + passes accessCode to the gate. gate1 GREEN (tsc 0, 119 tests, build ok); gate2 adversarial review 8/8 SHIP (button↔confirm keys, token routing, validation, deleted-code safety, JSX balance, ?emp= compat). BEHAVIOR CHANGE flagged: new ?emp= clicks now mint ?assess= unified assessments (drops open-text + workplace survey for NEW employee links). Awaiting owner go for merge+deploy.
- 14:36 UTC · s-0629-1143-d9be · B6 · shipped · owner approved "deploy as-is". PR #45 merged to main (fe773f4) + deployed CLEAN merged main to prod (detached origin/main; .env copied for build then removed). Live bundle index-Kjpp_WXz.js on hawkamah.web.app verified — B6 markers «إنشاء رابط الاستبيان» / «إنشاء رابط التقييم» / «إعدادات الأسئلة أدناه لا تؤثّر» / «إعداد استبيان البيئة» all present ×1. CI green. B6 🚢 SHIPPED. Owner-acknowledged behavior change live: تقييم الموظفين new-links are now MCQ-scored unified assessments (no workplace survey). Residual ⏳ = human check: create one link of each kind from the now-unified modal + confirm ?s= survey access-code/expiry.
